...
module.exports = class DB {
  ...
  findShownSoundCloudTracks(telegramUserId) {
    const collection = this.db.collection(DB_COLLECTIONS.SOUNDCLOUD_TRACKS);
    ...
    .toArray()
    .then((tracks) => {
      log.info('Find shown SoundCloud tracks success: ', telegramUserId, tracks);

      return tracks;
    }).catch((err) => {
      log.error('Find shown SoundCloud tracks: ', err, telegramUserId);
    ...
    return collection.findOne({
      ...
      if (doc) {
        return collection.update({
          ...
          log.error('Create SoundCloud track: ', err, track);
        });
      } else {
        track.shownToUsers = [telegramUserId];

        return collection.insertOne(track).catch((err) => {
          log.error('Create SoundCloud track: ', err, track);
        });
      }
    }).catch((err) => {
