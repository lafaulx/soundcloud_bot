...
module.exports = function(sc, db, tg) {
  function fetchTracksForUser(soundCloudUser) {
    return Promise.all([
      sc.getUserTracks(soundCloudUser.id, moment(new Date()).subtract(2, 'days')),
  ...
  function sendAudioLink(user, track) {
    return new Promise((resolve) => {
      tg.api.sendMessage(user.chatId, `[${track.user.username}](${track.user.permalink_url}) – `
      ...
      .catch(() => {
        db.suspendTelegramUser(user.id);
        resolve();
      });
    });
  }
